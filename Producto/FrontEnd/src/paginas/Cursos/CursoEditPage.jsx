...
function mapCursoAdminToForm(data) {
  return {
    nombre: data.nombre ?? "",
    ...
    horarios:
      data.horarios?.length > 0
        ? data.horarios.map((horario) => ({
            diaDeSemana: horario.diaDeSemana ?? "Lunes",
            horaDesde: (horario.horaDesde ?? "10:00:00").slice(0, 5),
            horaHasta: (horario.horaHasta ?? "12:00:00").slice(0, 5),
          }))
...
export default function CursoEditPage() {
  ...
  useEffect(() => {
    ...
    Promise.resolve().then(async () => {
      ...
      try {
        const res = await fetch(`/api/cursos/${id}/a`);
        if (res.status === 404) {
          if (!cancelled) {
            setForm(null);
            setLoading(false);
        ...
        if (!cancelled) {
          setForm(mapCursoAdminToForm(data));
        }
      } catch (err) {
        console.error(err);
        if (!cancelled) {
    ...
    return (
      <div className="container py-5">
        <div className="bg-white rounded-4 shadow-sm border p-5 text-center">
          ...
          <Link to={`/cursos/${id}`} className="btn btn-brand px-4">
            Volver al curso
          </Link>
        </div>
      </div>
