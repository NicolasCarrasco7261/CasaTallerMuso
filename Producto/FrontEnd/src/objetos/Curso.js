...
export function createCursoHorario({ diaDeSemana="Lunes", horaDesde="00:00:00", horaHasta="00:00:00" }) {
...
export function createCursoCard({ id="", nombre="", imagenStorageKey="", precio=-1, cupos=-1, cuposRestantes=-1 }) {
  const imagenUrl = apiUrlFromPath(`/api/files/img/${imagenStorageKey}`);
  return { id, nombre, imagenUrl, precio, cupos, cuposRestantes };
}

export function createCursoView({ nombre="", descripcion="", imagenStorageKey="", precio=-1, cupos=-1, cuposRestantes=-1, horarios=[] }) {
  const imagenUrl = apiUrlFromPath(`/api/files/img/${imagenStorageKey}`);
  const mappedHorarios = horarios.map((h) => createCursoHorario(h));
  return { nombre, descripcion, imagenUrl, precio, cupos, cuposRestantes, horarios: mappedHorarios };
}