...
export function createEventoHorario({ fecha="1970-01-01", hora="00:00:00" }) {
...
export function createEventoCard({ id="", nombre="", imagenStorageKey="", precio=-1, cupos=-1, cuposRestantes=-1 }) {
  const imagenUrl = apiUrlFromPath(`/api/files/img/${imagenStorageKey}`);
  return { id, nombre, imagenUrl, precio, cupos, cuposRestantes };
}

export function createEventoView({ nombre="", descripcion="", imagenStorageKey="", precio=-1, cupos=-1, cuposRestantes=-1, horarios=[] }) {
  const imagenUrl = apiUrlFromPath(`/api/files/img/${imagenStorageKey}`);
